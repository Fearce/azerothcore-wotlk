...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    // [WowPsParty PATCH] Pause AI so user input doesn't fight bot AI for the same body.
    //   Case A: someone is possessing this bot (CHARM_TYPE_POSSESS via .party swap)
    //           — user inputs drive *this* unit; the bot AI must shut up.
    //   Case B: this bot has a PlayerbotMgr (= it's the session-owning "master"
    //           character) AND isn't currently charming anything (= the session is
    //           driving its own body, not possessing somebody else). When the
    //           master .party swap's, GetCharm() becomes the possessed target,
    //           condition fails, and the AI is free to drive the vacated body.
    if (bot->HasUnitFlag(UNIT_FLAG_POSSESSED))
    {
        static thread_local std::unordered_map<uint32, uint32> lastPossLogMs;
        uint32 nowMs = getMSTime();
        uint32& last = lastPossLogMs[bot->GetGUID().GetCounter()];
        if (nowMs - last > 10000)
        {
            last = nowMs;
            LOG_INFO("module",
                "[WowPsParty] PlayerbotAI::UpdateAI PAUSED guid={} (UNIT_FLAG_POSSESSED set; user is controlling this body)",
                bot->GetGUID().GetCounter());
        }
        return;
    }
    if (sPlayerbotsMgr.GetPlayerbotMgr(bot) && !bot->GetCharm())
        return;

    // [WowPsParty PATCH] Managed party-of-5 bots get our rotation + combat
    // assist, then bail out — the rest of UpdateAI (default strategy engine,
    // action selection, etc.) must never run for them. This used to live
    // AFTER an "out-of-combat + has-directive" early return, which created
    // a deadlock: AssistTarget never ran out of combat, so the bot couldn't
    // make the first swing that would put it INTO combat. Now we run before
    // that gate. PartyFollow's MoveFollow re-asserter still drives non-
    // combat movement; AssistTarget handles combat engagement.
    extern bool WowPsParty_TickRotation_Trampoline(Player* bot);
    extern bool WowPsParty_BotHasActiveFollowDirective_Trampoline(ObjectGuid guid);
    extern void WowPsParty_AssistTarget_Trampoline(Player* bot);
    ...
    if (isPartyOfFiveBot)
    {
        ...
        // class-default rotation loaded on hire; alts get the player's
        // configured one. This is what stops henchmen melee-stacking like the
        // default playerbot AI does.
        WowPsParty_TickRotation_Trampoline(bot);
        WowPsParty_AssistTarget_Trampoline(bot);
        WowPsParty_TankFollowPath_Trampoline(bot);
        WowPsParty_TankLeadEngagement_Trampoline(bot);
...
void PlayerbotAI::UpdateAIInternal([[maybe_unused]] uint32 elapsed, bool minimal)
{
    ...
    // [WowPsParty DIAGNOSTIC] The default strategy engine must NEVER run for a
    // managed party-of-5 bot (UpdateAI hard-returns before calling us). If this
    // fires, the gate leaked — that's how an alt cast e.g. Shield Block despite
    // its rotation not containing it. Throttled per bot.
    {
        extern bool WowPsParty_BotHasActiveFollowDirective_Trampoline(ObjectGuid guid);
        if (WowPsParty_BotHasActiveFollowDirective_Trampoline(bot->GetGUID()))
        {
            ...
            if (now - last > 3000)
            {
                last = now;
                LOG_INFO("module",
                    "[WowPsParty GATE-LEAK] UpdateAIInternal ran for party bot "
                    "guid={} name={} combat={} — default AI engine executing!",
