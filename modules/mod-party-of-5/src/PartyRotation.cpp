...
namespace WowPsParty
{
                           ...
                           std::string const& flags, std::string const& cond)
    {
        ...
        if (verb == "move_behind")
        {
            ...
            // This verb exists ONLY to step out of the target's FRONTAL cone
            // (dragon breath, cleave) — it is NOT a backstab-positioning chase. So
            // it acts only while the bot is actually IN FRONT; anywhere out of the
            // front arc (side OR back) is already safe, so yield immediately and let
            // the bot cast from there. This is what stops the "kept moving behind,
            // never cast anything" loop: the old rule blocked the whole rotation
